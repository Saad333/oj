...
<?php include 'navbar.php'; ?>
		...
		if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true && $_SESSION['privilage'] < 9)
		{
			if (isset($_POST['submit'])) {
				...
				if(isset($_FILES['pdf'])){
				    ...
				    if($errors==''){
			    		$val = "http://localhost/ojtest/problems/" . $fi_name;
			    		unlink($val);
				       	move_uploaded_file($file_tmp,"http://localhost/ojtest/problems/".$file_name);
				    }
			    }
			    ...
			    elseif ($_POST["Select_test_case"]==1) {
			    	$sd = "http://localhost/ojtest/temp_code/input_files/" . $file_name;
					...
					$q = array();
			    	$v1 = $_POST['c_1_i_1'];
			    	$s0 = "http://localhost/ojtest/temp_code/input_files/" . $file_name;
			    	mkdir("$s0");
			    	$s1 = $s0 . '/1.txt';
			    	array_push($p, $s1);
	            ...
	            if ($result) {
	                ...
	                redirect_to("http://localhost/ojtest/modules/edit_program.php");
	            } else {
	                // Failure
	...
	?>
		            ...
		            <div class="container" style="margin-bottom: -40px;">
		            	<form class="form-horizontal" role="form" action="http://localhost/ojtest/modules/edit_program.php" method="post" enctype="multipart/form-data">
	    ...
	    <?php   }
			    	...
			    	if ($result) {
			    		// Success
			            $_SESSION["message"] = "Problem deleted";
			  			unlink($val);
			  			array_map('unlink', glob("$sd/*.*"));
						rmdir($sd);
	                    redirect_to("http://localhost/ojtest/modules/edit_program.php");
			    		...
			    		// Failure
			           $_SESSION["message"] = "Problem deletion failed";
	                    redirect_to("http://localhost/ojtest/modules/edit_program.php");
				                    ...
				                    <?php
				                    	...
				                    	foreach ($r_array as $key => $value) {
				                    		echo '<div class="row list-group-item panel-success" style="margin-right: 0px;margin-left: 0px;padding: 5px 15px;">';
						                    	...
						                    	echo '<div class="col-xs-2"><a href="http://localhost/ojtest/modules/edit_program.php?id='. $value['id'] .'&role=edit" style="vertical-align: middle;display: inline-block; padding: 5px 10px;" class="label label-warning"">Edit</a></div>';
						                    	echo '<div class="col-xs-2"><a href="http://localhost/ojtest/modules/edit_program.php?id='. $value['id'] .'&role=delete" style="vertical-align: middle;display: inline-block; padding: 5px;" class="label label-danger"">Delete</a></div>';
					                    	echo '</div>';
					                    	$i++;
				                    	}
				                    ?>
...
<?php       }
		...
		{
			redirect_to("http://localhost/ojtest/index.php");
		}?>
